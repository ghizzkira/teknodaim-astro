...
import { useUpdateGadget } from "@/hooks/useGadget"
...
interface EditGadgetFormProps {
  gadget: GadgetDataProps
}

const EditGadgetForm: React.FunctionComponent<EditGadgetFormProps> = (
  props,
) => {
  const { gadget } = props
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      wpTagSlug: gadget.wpTagSlug ?? "",
      wpCategorySlug: gadget.wpCategorySlug ?? "",
      networkTechnology: gadget.networkTechnology ?? "",
      launchAnnounced: gadget.launchAnnounced ?? "",
      launchStatus: gadget.launchStatus ?? "",
      bodyDimensions: gadget.bodyDimensions ?? "",
      bodyWeight: gadget.bodyWeight ?? "",
      bodySimCard: gadget.bodySimCard ?? "",
      displayType: gadget.displayType ?? "",
      displaySize: gadget.displaySize ?? "",
      displayResolution: gadget.displayResolution ?? "",
      platformOs: gadget.platformOs ?? "",
      platformChipset: gadget.platformChipset ?? "",
      platformCpu: gadget.platformCpu ?? "",
      platformGpu: gadget.platformGpu ?? "",
      memoryCardSlot: gadget.memoryCardSlot ?? "",
      memoryInternal: gadget.memoryInternal ?? "",
      mainCamera: gadget.mainCamera ?? "",
      mainCameraFeatures: gadget.mainCameraFeatures ?? "",
      mainCameraVideo: gadget.mainCameraVideo ?? "",
      selfieCamera: gadget.selfieCamera ?? "",
      selfieCameraVideo: gadget.selfieCameraVideo ?? "",
      soundLoudspeaker: gadget.soundLoudspeaker ?? "",
      sound35mmJack: gadget.sound35mmJack ?? false,
      commsWlan: gadget.commsWlan ?? "",
      commsPositioning: gadget.commsPositioning ?? "",
      commsNfc: gadget.commsNfc ?? "",
      commsRadio: gadget.commsRadio ?? false,
      commsUsb: gadget.commsUsb ?? "",
      featuresSensors: gadget.featuresSensors ?? "",
      batteryType: gadget.batteryType ?? "",
      batteryCharging: gadget.batteryCharging ?? "",
      miscColors: gadget.miscColors ?? "",
      miscModels: gadget.miscModels ?? "",
      miscSar: gadget.miscSar ?? "",
      miscSarEu: gadget.miscSarEu ?? "",
      miscPrice: gadget.miscPrice ?? "",
      metaTitle: gadget.metaTitle ?? "",
      metaDescription: gadget.metaDescription ?? "",
      status: gadget?.status ?? ("draft" as const),
      featuredImageId: gadget.featuredImageId ?? "",
    },
  })

  const { handleUpdateGadget: updateGadget } = useUpdateGadget({
    onSuccess: () => {
      form.reset()
