...
const InfiniteScrollWpPost: React.FunctionComponent<
  InfiniteScrollWpPostProps
> = (props) => {
  const { posts, pageInfo, language, filteredQueries } = props

  const loadMoreRef = React.useRef<HTMLDivElement>(null)
  const [page, setPage] = React.useState<WPPageInfoProps>(
    pageInfo
      ? pageInfo
      : { endCursor: "", hasNextPage: true, offsetPagination: { total: 10 } },
  )
  const [list, setList] = React.useState<WpSinglePostDataProps[]>(
    posts ? posts : [],
  )
...
export default InfiniteScrollWpPost
