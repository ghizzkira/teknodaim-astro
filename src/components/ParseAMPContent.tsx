...
interface ParseAmpContentProps {
  htmlInput: string
  title: string
}

const ParseAmpContent: React.FunctionComponent<ParseAmpContentProps> = (
  props,
) => {
  ...
  const processingInstructions = [
    {
      ...
      processNode: function (
        ...
        if (node.attribs?.class?.includes("twitter-tweet")) {
          return (
            <TwitterAMPEmbed key={index + title + "blockquote"}>
              {children}
            </TwitterAMPEmbed>
          )
        }
...
export default ParseAmpContent
